...
#+description: Learn how to audit AWS S3 buckets for public access.
...
*Condition:* S3 bucket ~13bf5920-a09f-47bc-a75a-394a09f18d6a~ does not have all four
Public Access Block flags enabled at the bucket level (~PAB_FullyRestricted:
FALSE-VULNERABLE~).

*Criteria:* All S3 buckets should have Public Access Block fully enabled at the
bucket level to prevent accidental public exposure through bucket policies or
ACLs.
...
*Evidence:* Attach ~s3_full_public_access_audit.csv~ filtered to the specific
bucket. The ~PAB_FullyRestricted~ column documents the current state.

*Finding 2: Bucket actively accessible to the public*

*Condition:* S3 bucket ~example-bucket~ has an ACL granting ~READ~ access to the
~AllUsers~ group (~ACL_AllUsersRead: TRUE~, ~OverallPublicStatus: TRUE - ACL Read~).

*Criteria:* S3 buckets should not be publicly accessible unless there is a
documented business requirement and the bucket has been reviewed and approved
for public access.
...
*Evidence:* Attach ~s3_full_public_access_audit.csv~ filtered to the specific
bucket. For higher-risk buckets, supplement with a screenshot from the AWS
console confirming the bucket contents or data classification.
